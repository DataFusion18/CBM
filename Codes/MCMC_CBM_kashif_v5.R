...
GPP.data = rnorm2(length(time),1,0.4) # Generate GPP data with mean=15, sd=3
GPP.data = GPP.data[order(GPP.data)]
  Rd.data = rnorm2(length(time),0.04,0.008) # Generate Rd data with mean=0.04, sd=0.008  # Units gC g-1C d-1
# Generate Cstorage.data to create a measurement sets of Cleaf, Cstem, Croot 
# Cstorage.data = rnorm2(length(time),7.5,2) # Generate Cstorage data with mean=7.5, sd=2
Y = 0.3

# Generate random parameter sets for synthetic Cleaf,Cstem,Croot data generation
k.data = rnorm2(length(time),0.5,0.05) # Generate data 'k' values with mean=0.55, sd=0.15 
af.data = rnorm2(length(time),1/3,0.1) # Generate data 'af' values with mean=1/7, sd=0.05 
as.data = rnorm2(length(time),1/3,0.1) # Generate data 'as' values with mean=3/7, sd=0.15
sf.data = rnorm2(length(time),1/100,1/1000) # Generate data 'sf' values with mean=1/30, sd=1/100
...
# Generating and plotting the data (synthetic) data sets
...
matplot(output.data,type = c("b"),pch=1,col = 1:4,xlab="Days",ylab="gC",main="Different Carbon pools") #plot
legend("topleft", legend = c("Cstorage.data","Cleaf.data","Cstem.data","Croot.data"), col=1:4, pch=0.75) # optional legend
...
param.sf <- matrix(c(0,1/1000,1/100) , nrow=no.param, ncol=3, byrow=T) 
...
# Calculating model outputs for the starting point of the chain and then the likelihood
# Cstorage <- matrix(, nrow=length(GPP.data), ncol=1)
# Cleaf <- matrix(, nrow=length(GPP.data), ncol=1)
# Cstem <- matrix(, nrow=length(GPP.data), ncol=1)
# Croot <- matrix(, nrow=length(GPP.data), ncol=1)
...
{logli[i] = - 0.5*((output$Cleaf[i] - output.data$Cleaf.data[i])/sd.Cleaf)^2
- 0.5*((output$Cstem[i] - output.data$Cstem.data[i])/sd.Cstem)^2 - 0.5*((output$Croot[i] - output.data$Croot.data[i])/sd.Croot)^2 
- log(sd.Cleaf) - log(sd.Cstem) - log(sd.Croot)}
# {logli[i] = -0.5*((output$Cstorage[i] - Cstorage.data[i])/sd.Cstorage)^2 - log(sd.Cstorage) - 0.5*((output$Cleaf[i] - Cleaf.data[i])/sd.Cleaf)^2
# - 0.5*((output$Cstem[i] - Cstem.data[i])/sd.Cstem)^2 - 0.5*((output$Croot[i] - Croot.data[i])/sd.Croot)^2 
# - log(sd.Cleaf) - log(sd.Cstem) - log(sd.Croot)} 
...
if (Prior1 > 0){
  ...
  for (i in 1:length(time))
  {logli[i] = -0.5*((out.cand$Cleaf[i] - output.data$Cleaf.data[i])/sd.Cleaf)^2
  - 0.5*((out.cand$Cstem[i] - output.data$Cstem.data[i])/sd.Cstem)^2 - 0.5*((out.cand$Croot[i] - output.data$Croot.data[i])/sd.Croot)^2 
  - log(sd.Cleaf) - log(sd.Cstem) - log(sd.Croot)} 
  # {logli[i] = -0.5*((out.cand$Cstorage[i] - Cstorage.data[i])/sd.Cstorage)^2 - log(sd.Cstorage) - 0.5*((out.cand$Cleaf[i] - Cleaf.data[i])/sd.Cleaf)^2
  # - 0.5*((out.cand$Cstem[i] - Cstem.data[i])/sd.Cstem)^2 - 0.5*((out.cand$Croot[i] - Croot.data[i])/sd.Croot)^2 
  # - log(sd.Cleaf) - log(sd.Cstem) - log(sd.Croot)} 
  logL1 <- sum(logli)
...
corrMatrix.1 = cor(output.final$Cleaf,output.data$Cleaf)
t1 = (paste("Correlation Coefficient, r2 of original Cleaf measurements and predictions: ", corrMatrix.1*corrMatrix.1))
corrMatrix.2 = cor(output.final$Cstem,output.data$Cstem)
t2 = (paste("Correlation Coefficient, r2 of original Cstem measurements and predictions: ", corrMatrix.2*corrMatrix.2))
corrMatrix.3 = cor(output.final$Croot,output.data$Croot)
t3 = (paste("Correlation Coefficient, r2 of original Croot measurements and predictions: ", corrMatrix.3*corrMatrix.3))
# corrMatrix.4 = cor(output.final$Cstorage,output.data$Cstorage)
# t4 = (paste("Correlation Coefficient, r2 of original Cstorage measurements and predictions: ", corrMatrix.4*corrMatrix.4))
print(t1)
print(t2)
print(t3)
# print(t4)
...
# plot(Cleaf.data, output.final$Cleaf, main="Scatterplot Example", 
#      xlab="data Cleaf", ylab="Estimated Cleaf", pch=19)
# plot(Cleaf.data, output.final$Cleaf, main="Scatterplot Example", 
#      xlab="data Cleaf", ylab="Estimated Cleaf", pch=19)
# abline(lm(output.final$Cleaf~Cleaf.data), col="red") # regression line (y~x)
# plot(Cstem.data, output.final$Cstem, main="Scatterplot Example", 
#      xlab="data Cstem", ylab="Estimated Cstem", pch=19)
# abline(lm(output.final$Cstem~Cstem.data), col="red") # regression line (y~x)
# 
# lines(lowess(Cleaf.data, output.final$Cleaf), col="blue") # lowess line (x,y)
# 
# 
# 
# 
# 
# 
# t=paste("Parameter Number / data Parameter Values / Estimated Parameter Values:");
# print(t)
# for (i in 1:length(time)){
#   t = paste (i, ": ", k.data[i], "    ", format(round(param.final[i], 2),, nsmall = 2) );
#   print(t)
# }  
